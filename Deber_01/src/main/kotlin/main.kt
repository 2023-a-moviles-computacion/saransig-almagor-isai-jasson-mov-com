...
fun main(){
    ...
    var crudProductos = CRUDProductos(archivoP)
...
fun menuProducto(crudProductos: CRUDProductos, idTienda: Int){
    ...
    while(flag){
        ...
        when(opcion){
            ...
            "2" -> mostrarProductos(crudProductos, idTienda)
            "2" -> mostrarProductos(crudProductos, idTienda)
            "3" -> actualizarProducto(crudProductos, idTienda)
            "4" -> eliminarProducto(crudProductos, idTienda)
            "0" -> flag = false
            else -> println("Opción NO valida")
        }
    }
...
fun eliminarProducto(crudProductos: CRUDProductos, idTienda: Int){
    println("Ingrese el ID del producto que quiere eliminar:")
    val idProducto = readLine()?.toIntOrNull() ?: 0
    val producto = crudProductos.productoExiste(idProducto, idTienda)
    crudProductos.eliminarProducto(producto)
}

fun mostrarProductos(crudProductos: CRUDProductos, idTienda: Int){
    crudProductos.mostrarProductos(idTienda)
}

fun actualizarProducto(crudProductos: CRUDProductos, idTienda: Int) {
    println("Ingrese el ID del Producto que quiere modificar")
    var idProducto = readln().toInt()

    var producto: Producto? = crudProductos.productoExiste(idProducto, idTienda)
    if (producto != null) {
        println("------------------------------------")
        ...
        println("No existe producto con ID ingresado para esta tienda")
    }
